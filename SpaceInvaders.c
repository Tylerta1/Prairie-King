...
void Print_Image(Character_t *character){
	ST7735_DrawBitmap(character->xpos, character->ypos, character->image, character->width, character->height);
}
void clear(Sprite_t *sprite){
	ST7735_FillRect(character->xpos, character->ypos-character->height+1, character->width, character->height, 0xFFFF);
}
void Print_Bullet(Bullet_t *bullet){
	ST7735_DrawBitmap(bullet->xpos, bullet->ypos, bullet->image, bullet->width, bullet->height)
	}
}
